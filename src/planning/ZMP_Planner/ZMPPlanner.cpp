...
void ZMPPlanner::getNextZMP( Vector2d tP_B, double tth_B )
{
  ...
  */
  ...
  if( status == STOP ) {
	...
	refzmp_list.push_back( Vector3d(foot_time+preview_delay, tP_B[0], tP_B[1]) );
  }
}
...
void ZMPPlanner::plot_foot_pattern()
{
  ...
  fprintf( gp, "plot '-' with lines lw 2 lc rgb \"blue\" title \"reference ZMP\", '-' with points pt 2 lcrgb \"red\" title \"Torso Position\"\n");
